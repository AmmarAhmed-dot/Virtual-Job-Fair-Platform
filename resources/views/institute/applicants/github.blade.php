<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-4">
            <a href="javascript:history.back()" class="text-gray-500 hover:text-gray-700 transition">
                ...
                <div
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <a href="https://github.com/{{ $analysis['username'] }}" target="_blank"
                                class="text-xs text-indigo-400 hover:text-indigo-300 font-semibold flex items-center mt-0.5">
                    ...
                    <div class="bg-slate-800/40 p-4 rounded-xl border border-slate-800 flex flex-col justify-between">
                        ...
                        <div class="flex items-baseline space-x-1">
                            <p class="text-2xl font-black text-indigo-400">
                                {{ $analysis['contributions'] ?? $analysis['commits'] ?? 0 }}
                            </p>
                            <span class="text-[9px] text-slate-500">({{ $analysis['commits'] ?? 0 }} c /
                                {{ $analysis['issues_prs'] ?? 0 }} pr)</span>
                        </div>
                    </div>
                    <div class="bg-slate-800/40 p-4 rounded-xl border border-slate-800 flex flex-col justify-between">
                        ...
                        <div class="flex items-baseline space-x-1">
                            ...
                            @if($analysis['has_token'])
                                <span class="text-[9px] text-slate-500">({{ $analysis['public_repos'] }} pub /
                                    {{ $analysis['private_repos'] }} priv)</span>
                            @else
                    ...
                    <div class="bg-slate-800/40 p-4 rounded-xl border border-slate-800 flex flex-col justify-between">
                        ...
                        <div class="flex items-baseline space-x-1">
                            <p class="text-2xl font-black text-yellow-400 flex items-center">
                                <svg class="w-4.5 h-4.5 mr-1 text-yellow-400 fill-current" viewBox="0 0 20 20">
                                    <path
                        ...
                        <div class="md:col-span-5">
                            ...
                            <div class="flex flex-wrap gap-1.5">
                                @foreach($analysis['languages'] as $lang => $count)
                                    <div
                                        class="bg-indigo-500/10 border border-indigo-500/20 px-2.5 py-1 rounded-xl flex items-center space-x-1.5 text-xs font-semibold text-indigo-300">
